...
def main(batch_size, model_name, lr, l2, epoch, patience, warmup_steps):
    model = DistilBertForSequenceClassification.from_pretrained(model_name, num_labels=2)

    trainData, evalData, testData = utils.read_dataset('rte', 'rte_seedData.pkl')

    train_additional_1 = pickle.load(open("./Datasets/rte/rte_additional-1.pkl", 'rb'))
    train_additional_2 = pickle.load(open("./Datasets/rte/rte_additional-2.pkl", 'rb'))

    for i in range(len(trainData)):
        trainData[i] += train_additional_1[i]
        trainData[i] += train_additional_2[i]
    ...
    pickle.dump(misClassData, open("./misclass_data/rte/distilBert_misclass.pkl", 'wb'))
